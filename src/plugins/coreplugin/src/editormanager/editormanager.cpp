...
{
    CustomEdit *editor = new CustomEdit(name);
...
CustomEdit *EditorManager::createEditorFromFile(const QString &filePath)
{
    CustomEdit *editor = CustomEdit::fromFile(filePath);
...
CustomEdit *EditorManager::cloneEditor(CustomEdit *editor)
{
    CustomEdit *clonedEditor = new CustomEdit("Clone");
...
CustomEdit *EditorManager::getEditorByFilePath(const QString &filePath)
{
...
void EditorManager::manageEditor(CustomEdit *editor)
{
    editors.append(QPointer<CustomEdit>(editor));
}

void EditorManager::setupEditor(CustomEdit *editor)
{
    qInfo(Q_FUNC_INFO);
...
{
    QMutableListIterator<QPointer<CustomEdit>> it(editors);

    while (it.hasNext()) {
        QPointer<CustomEdit> pointer = it.next();
        if (pointer.isNull())
            it.remove();
    }
}
